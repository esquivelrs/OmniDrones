...
@hydra.main(version_base=None, config_path=".", config_name="demo_cf")
def main(cfg):
    ...
    translations[:, 0] = 0.0
    translations[:, 1] = 0.0 #torch.arange(n)
    translations[:, 2] = 0.5
    ...
    #controller = DSLPIDController(dt=0.01,g=9.81, uav_params=drone.params).to(sim.device)
    ...
    for i in tqdm(range(1000)):
        ...
        action = -0.1295*torch.ones([4], device=sim.device)
